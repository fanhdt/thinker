...
async def _handle_update(client: TelegramClient, update: dict) -> None:
    ...
    async with async_session_factory() as session:
        try:
            ...
            if exc.retryable:
                user_message = (
                    "Lagi kena limit dari layanan AI-nya (rate limit). "
                    "Coba kirim lagi dalam 1 menit yaa."
                )
            else:
                user_message = "Maaf ada gangguan sementara di sisi saya. Coba lagi sebentar yaa."
